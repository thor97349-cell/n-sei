        ...
        <div className="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {STEPS.map((step, i) => (
            <div key={step.title} className="relative flex flex-col items-center text-center">
              <div className="flex h-16 w-16 items-center justify-center rounded-full bg-brand/10 text-2xl ring-1 ring-brand/30">
                {step.icon}
              </div>
              ...
              {i < STEPS.length - 1 && (
                <span
                  aria-hidden
                  className="absolute top-8 left-[calc(50%+2.5rem)] hidden h-px w-[calc(100%-5rem)] bg-brand/30 lg:block"
                />
              )}
            </div>
          ))}
